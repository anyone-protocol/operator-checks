...
export class DistributionChecksService {
  ...
  constructor(
    ...
    this.relayRewardsOperatorMinAOBalance = this.config.get<number>('RELAY_REWARDS_OPERATOR_MIN_AO_BALANCE', {
      infer: true,
    })
    this.relayRewardsOperatorMaxAOBalance = this.config.get<number>('RELAY_REWARDS_OPERATOR_MAX_AO_BALANCE', {
      infer: true,
    })
    this.stakingRewardsOperatorMinAOBalance = this.config.get<number>('STAKING_REWARDS_OPERATOR_MIN_AO_BALANCE', {
      infer: true,
    })
    this.stakingRewardsOperatorMaxAOBalance = this.config.get<number>('STAKING_REWARDS_OPERATOR_MAX_AO_BALANCE', {
      infer: true,
    })
  }
  ...
  async getRelayRewardsOperatorBalance(): Promise<{
    ...
    try {
      ...
      if (balance.lt(this.relayRewardsOperatorMinAOBalance)) {
        this.logger.warn(
          `Balance depletion on relay rewards operator: ${balance} $AO < ${this.relayRewardsOperatorMinAOBalance} $AO`,
        )
        ...
        this.logger.warn(
          `[alarm=balance-accumulation-ao-relay-rewards] Balance accumulation on relay rewards operator: ${balance} $AO > ${this.relayRewardsOperatorMaxAOBalance} $AO`,
        )
      } else {
      ...
      this.logger.error(`Exception while fetching relay rewards operator $AO balance`, error.stack)
    }
  ...
  async getStakingRewardsOperatorBalance(): Promise<{
    ...
    try {
      ...
      if (balance.lt(this.stakingRewardsOperatorMinAOBalance)) {
        this.logger.warn(
          `Balance depletion on staking rewards operator: ${balance} $AO < ${this.stakingRewardsOperatorMinAOBalance} $AO`,
        )
        ...
        this.logger.warn(
          `[alarm=balance-accumulation-ao-staking-rewards] Balance accumulation on staking rewards operator: ${balance} $AO > ${this.stakingRewardsOperatorMaxAOBalance} $AO`,
        )
      } else {
      ...
      this.logger.error(`Exception while fetching staking rewards operator $AO balance`, error.stack)
    }
